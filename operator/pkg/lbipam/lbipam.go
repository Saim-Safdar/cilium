...
func (ipam *LBIPAM) Start(_ hive.HookContext) error {
	...
	// Start go routine, we can't block in Start
	go func() {
		defer ipam.wg.Done()
		ipam.Run()
	}()
...
func (ipam *LBIPAM) Stop(ctx hive.HookContext) error {
	...
	// Convert blocking Wait into a channel so we can select on the context closing or the Wait.
...
func (ipam *LBIPAM) restart() {
	...
	// Refresh the context
	ipam.ctx, ipam.cancel = context.WithCancel(context.Background())
...
func (ipam *LBIPAM) Run() {
	...
	poolChan := stream.ToChannel[ipPoolEvent](ipam.ctx, errChan, ipam.poolResource)
	svcChan := stream.ToChannel[svcEvent](ipam.ctx, errChan, ipam.svcResource)

	ipam.logger.Info("LB-IPAM initializing")
	err = ipam.init(poolChan, svcChan)
	if err != nil {
...
func (ipam *LBIPAM) preInit() error {
	preInitCtx, cancel := context.WithCancel(ipam.ctx)
	defer cancel()
...
func (ipam *LBIPAM) init(
	...
	if err := ipam.initSyncPools(poolChan); err != nil {
		return err
	}

	// Then sync all services
	if err := ipam.initSyncSvc(svcChan); err != nil {
		return err
	}
...
func (ipam *LBIPAM) initSyncPools(poolChan <-chan ipPoolEvent) error {
	type retry struct {
		cnt int
		fn  func() error
	}
	...
	for !poolSync {
		...
		case event := <-poolChan:
			event.Handle(
				func() error {
					err := ipam.settleConflicts()
					if err != nil {
						ipam.logger.WithError(err).Error("Error while settling pool conflicts")
						poolRetryQueue.Add(retry{fn: ipam.settleConflicts})
					}
				...
				func(k resource.Key, pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
					err := ipam.handleNewPool(pool)
					if err != nil {
						ipam.logger.WithError(err).Error("Error while handling new pools")
						poolRetryQueue.Add(retry{fn: func() error { return ipam.handleNewPool(pool) }})
					}
...
func (ipam *LBIPAM) initSyncSvc(svcChan <-chan svcEvent) error {
	svcSync := false
	for !svcSync {
		select {
		case event := <-svcChan:
			event.Handle(
				func() error {
					...
					err := ipam.satisfyServices()
					if err != nil {
						// Let the resource retry after the initialization
						return err
					}

					err = ipam.updateAllPoolCounts()
					if err != nil {
						// Let the resource retry after the initialization
						return err
					}
...
func (ipam *LBIPAM) poolOnUpsert(k resource.Key, pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	...
	if _, exists := ipam.pools[pool.GetName()]; exists {
		...
		err = ipam.handlePoolModified(pool)
		if err != nil {
			ipam.logger.WithError(err).Error("Error in handlePoolModified()")
		}
	} else {
		ipam.logger.Debugf("Adding pool '%s'", pool.GetName())
		err = ipam.handleNewPool(pool)
		if err != nil {
			ipam.logger.WithError(err).Error("Error in handleNewPool()")
		}
	}
	if err != nil {
		return err
	...
	err = ipam.settleConflicts()
	if err != nil {
		ipam.logger.WithError(err).Error("Error in settleConflicts()")
		return fmt.Errorf("settleConflicts: %w", err)
	}

	err = ipam.satisfyAndUpdateCounts()
	if err != nil {
		ipam.logger.WithError(err).Error("Error in satisfyAndUpdateCounts()")
...
func (ipam *LBIPAM) poolOnDelete(k resource.Key, pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	err := ipam.handlePoolDeleted(pool)
	if err != nil {
		ipam.logger.WithError(err).Error("Error in handlePoolDeleted()")
...
func (ipam *LBIPAM) svcOnUpsert(k resource.Key, svc *slim_core_v1.Service) error {
	...
	err := ipam.handleUpsertService(svc)
	if err != nil {
		ipam.logger.WithError(err).Error("Error in handleUpsertService()")
...
func (ipam *LBIPAM) svcOnDelete(k resource.Key, svc *slim_core_v1.Service) error {
	...
	if err != nil {
		ipam.logger.WithError(err).Error("Error in handleDeletedService()")
...
func (ipam *LBIPAM) satisfyAndUpdateCounts() error {
	err := ipam.satisfyServices()
	if err != nil {
...
func (ipam *LBIPAM) satisfyServices() error {
...
func (ipam *LBIPAM) handleNewPool(pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	...
	ipam.unmarkPool(pool)

	return nil
}

func (ipam *LBIPAM) handlePoolModified(pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	ipam.pools[pool.GetName()] = pool
	...
	for _, extRange := range existingRanges {
		...
		err := ipam.deleteRangeAllocations(extRange)
		if err != nil {
	...
	err := ipam.revalidateAllServices()
	if err != nil {
...
func (ipam *LBIPAM) revalidateAllServices() error {
...
func (ipam *LBIPAM) updateAllPoolCounts() error {
...
func (ipam *LBIPAM) deleteRangeAllocations(delRange *LBRange) error {
...
func (ipam *LBIPAM) handlePoolDeleted(pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	...
	for _, poolRange := range poolRanges {
		...
		err := ipam.deleteRangeAllocations(poolRange)
		if err != nil {
			return fmt.Errorf("deleteRangeAllocations: %w", err)
		}
	}
...
func (ipam *LBIPAM) settleConflicts() error {
	...
	for _, poolOuter := range ipam.pools {
		...
		if conflicting, rangeA, rangeB := areRangesInternallyConflicting(outerRanges); conflicting {
			err := ipam.markPoolConflicting(poolOuter, poolOuter, rangeA, rangeB)
			if err != nil {
		...
		for _, poolInner := range ipam.pools {
			...
			if conflicting, outerRange, innerRange := areRangesConflicting(outerRanges, innerRanges); conflicting {
				...
				if poolOuter.CreationTimestamp.Before(&poolInner.CreationTimestamp) {
					err := ipam.markPoolConflicting(poolInner, poolOuter, innerRange, outerRange)
					if err != nil {
						return fmt.Errorf("markPoolConflicting: %w", err)
					}
					break
				...
				err := ipam.markPoolConflicting(poolOuter, poolInner, outerRange, innerRange)
				if err != nil {
					return fmt.Errorf("markPoolConflicting: %w", err)
				}
				break
...
func (ipam *LBIPAM) markPoolConflicting(
	...
	if ipam.setPoolCondition(targetPool, ciliumPoolConflict, meta_v1.ConditionTrue, "cidr_overlap", conflictMessage) {
		err := ipam.patchPoolStatus(targetPool)
		if err != nil {
			return fmt.Errorf("patchPoolStatus: %w", err)
		}
	}
...
func (ipam *LBIPAM) unmarkPool(targetPool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
...
func (ipam *LBIPAM) patchSvcStatus(sv *ServiceView) error {
	...
	_, err = ipam.svcClient.Services(sv.Key.Namespace).Patch(ipam.ctx, sv.Key.Name,
...
func (ipam *LBIPAM) patchPoolStatus(pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	replaceSvcStatus := []k8s.JSONPatch{
		{
			OP:    "replace",
			Path:  "/status",
	...
	_, err = ipam.poolClient.Patch(ipam.ctx, pool.Name,
		types.JSONPatchType, createStatusPatch, meta_v1.PatchOptions{
			FieldManager: ciliumFieldManager,
		}, "status")
